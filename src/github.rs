use crate::{
    FILE_LIMIT, MANIFEST_LIMIT, OmapackError, PackageFile, PackageManifest, Source, strict_json,
};
use base64::Engine;
...
impl GitHubClient {
    pub fn new(token: Option<&str>) -> Result<Self, OmapackError> {
        let mut headers = HeaderMap::new();
        headers.insert(USER_AGENT, HeaderValue::from_static("omapack/0.1"));
        headers.insert(
            ACCEPT,
        ...
        if let Some(token) = token.filter(|token| !token.is_empty()) {
            let value = HeaderValue::from_str(&format!("Bearer {token}"))
                .map_err(|_| OmapackError("GITHUB_TOKEN contains invalid header bytes".into()))?;
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|error| OmapackError(format!("cannot create HTTPS client: {error}")))?;
        Ok(Self {
            client,
            trees: BTreeMap::new(),
        })
    ...
    pub fn resolve(&mut self, source: Source) -> Result<ResolvedPackage, OmapackError> {
        ...
        if manifest_entry.mode != "100644" {
            return Err(OmapackError(
                "omapack.json must be a regular non-executable file".into(),
            ));
        }
    ...
    fn descend_tree(
        ...
        for component in path.split('/') {
            ...
            let entry = entries
                ...
                .ok_or_else(|| OmapackError(format!("package directory not found: {path}")))?;
    ...
    fn file_entry(
        ...
        while let Some(component) = components.next() {
            ...
            let entry = entries
                .iter()
                .find(|entry| entry.path == component)
                .ok_or_else(|| OmapackError(format!("declared file not found: {path}")))?;
        ...
        Err(OmapackError(format!("invalid empty file path: {path}")))
    }
    ...
    fn tree(
        ...
        repository: &str,
        sha: &str,
    ) -> Result<Vec<TreeEntry>, OmapackError> {
    ...
    fn blob(
        ...
        entry: &TreeEntry,
        limit: usize,
    ) -> Result<Vec<u8>, OmapackError> {
        let size = entry
            .size
            .ok_or_else(|| OmapackError("Git blob is missing its size".into()))?;
        if size > limit as u64 {
            return Err(OmapackError(format!("Git blob exceeds {limit} bytes")));
        }
        ...
        let content = BASE64
            .decode(compact)
            .map_err(|_| OmapackError("GitHub returned invalid Base64 blob content".into()))?;
    ...
    fn api_json<T: for<'de> Deserialize<'de>>(
        ...
        path: &[&str],
        limit: usize,
    ) -> Result<T, OmapackError> {
        let mut url = Url::parse("https://api.github.com/")
            .map_err(|error| OmapackError(error.to_string()))?;
        url.path_segments_mut()
            .map_err(|_| OmapackError("cannot construct GitHub API URL".into()))?
            .extend(path);
        let response = self
            .client
            .get(url)
            .send()
            .map_err(|error| OmapackError(format!("GitHub request failed: {error}")))?;
        let bytes = bounded_response(response, limit)?;
        strict_json(&bytes, limit)
    }
}

fn bounded_response(mut response: Response, limit: usize) -> Result<Vec<u8>, OmapackError> {
    ...
    response
        ...
        .map_err(|error| OmapackError(format!("cannot read GitHub response: {error}")))?;
    if bytes.len() > limit {
        return Err(OmapackError(format!(
            "GitHub response exceeds {limit} bytes"
        )));
