...
canonical-digest contracts with little code. Omapack, however, is intended to
...
Implement Omapack as one Rust CLI crate producing a standalone binary. Use
...
Omapack's execution path.

## Consequences

Omapack can ship as one architecture-specific release asset and later through
...
or run Omapack when using an already self-contained Omarchy plugin.
